...
export function createScene(host:HTMLElement,onSelect:(id:string)=>void,onManual:()=>void):SceneAPI{
 ...
 const glowCanvas=document.createElement('canvas');glowCanvas.width=128;glowCanvas.height=128;const gc=glowCanvas.getContext('2d')!;gc.fillStyle='#000';gc.fillRect(0,0,128,128);gc.fillStyle='#ffcc7a';gc.fillRect(20,16,88,86);const glowTexture=new T.CanvasTexture(glowCanvas);glowTexture.wrapS=glowTexture.wrapT=T.RepeatWrapping;glowTexture.colorSpace=T.SRGBColorSpace;
 ...
 function inst(m:T.Material,x:number,y:number,z:number,w:number,h:number,d:number){dummy.position.set(x,y,z);dummy.rotation.set(0,0,0);dummy.scale.set(w,h,d);dummy.updateMatrix();if(!instances.has(m))instances.set(m,[]);instances.get(m)!.push(dummy.matrix.clone())}
 ...
 const boats:T.Group[]=[];for(let i=0;i<7;i++){const b=new T.Group();b.name='River boat '+(i+1);box(b,0,0,0,2.8,1,9,trim);box(b,0,.8,-.5,2.2,1,5,stone);box(b,0,1.5,-1,1.8,.6,3,glass);box(b,0,1.2,2,1.5,.4,1,gold);const bow=mesh(b,new T.ConeGeometry(1.5,2,3),trim,0,0,-5);bow.rotation.x=-Math.PI/2;const wake=box(b,0,-.2,8,3.2,.03,5,mat('#91bdc2',.1));wake.name='Simple wake';world.add(b);boats.push(b)}
 ...
 function resize(){renderer.setSize(host.clientWidth,host.clientHeight);camera.aspect=host.clientWidth/host.clientHeight;camera.updateProjectionMatrix()}const ro=new ResizeObserver(resize);ro.observe(host);
