...
export function createScene(host:HTMLElement,onSelect:(id:string)=>void,onManual:()=>void):SceneAPI{
 ...
 const refinement=addRefinements(world,scene,groups);for(const g of groups.values())g.traverse(o=>{if(o instanceof T.Mesh&&!pickables.includes(o))pickables.push(o)});
 ...
 refinement.update(time,windowMat.emissiveIntensity);wakeMaterials.forEach((m,i)=>m.opacity=.14+Math.sin(time*1.5+i)*.035);
