...
export function createScene(host:HTMLElement,onSelect:(id:string)=>void,onManual:()=>void):SceneAPI{
 ...
 const clouds:T.Sprite[]=[];for(let i=0;i<18;i++){const c=new T.Sprite(cloudMaterial);const a=i*Math.PI*2/18;c.position.set(Math.cos(a)*780,230+(i%4)*45,Math.sin(a)*780);c.scale.set(260+(i%3)*60,110+(i%2)*30,1);c.name='Drifting white cloud';scene.add(c);clouds.push(c)}
 ...
 let lightMode='day';const themes:Record<string,{bg:string;water:string;sun:string;ambient:number;power:number;emission:number}>={day:{bg:'#68a9e4',water:'#688d96',sun:'#fff5e5',ambient:1.55,power:2.8,emission:0},sunset:{bg:'#d4a695',water:'#767269',sun:'#ffad65',ambient:1.6,power:3.1,emission:.45},night:{bg:'#07172f',water:'#173b54',sun:'#8eb8ff',ambient:.3,power:.4,emission:.78}};
