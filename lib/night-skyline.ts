...
export function addNightSkyline(world:T.Group,groups:Map<string,T.Group>){
 ...
 return {update(n:number){accents.forEach(m=>m.emissiveIntensity=n*2.1);surfaceLights.forEach(({m,strength})=>m.emissiveIntensity=n*strength);screenMaterial.emissiveIntensity=n*1.55;screen.visible=n>.05;}};
}
