...
export function addRefinements(world:T.Group,scene:T.Scene,groups:Map<string,T.Group>){
 ...
 const walkingRoutes=mapData.roads.filter(r=>['footway','pedestrian'].includes(r.kind)&&r.points.length>=2&&!r.bridge&&r.points.some(near)&&clearWalkingPath(r.points)).filter(r=>r.points.slice(1).reduce((n,p,i)=>n+Math.hypot(p[0]-r.points[i][0],p[1]-r.points[i][1]),0)>1).sort((a,b)=>Math.min(...a.points.map(p=>p[1]))-Math.min(...b.points.map(p=>p[1]))).slice(0,160);
 const pedestrianTarget=400;
 for(const r of walkingRoutes){
  ...
  const routePopulation=Math.floor(pedestrianTarget/walkingRoutes.length)+(pathIndex<pedestrianTarget%walkingRoutes.length?1:0);
  for(let i=0;i<routePopulation;i++){const g=new T.Group();g.name='Pedestrian';box(g,0,.2,0,.11,.24,.08,clothes[(pathIndex+i)%5]);sphere(g,0,.39,0,.065,limestone);for(const x of [-.035,.035])box(g,x,.065,0,.035,.13,.04,unlitWindow);g.userData.pathIndex=pathIndex;g.userData.phase=i%2?2-(i+.5)/routePopulation:(i+.5)/routePopulation;g.userData.length=points.slice(1).reduce((n,p,j)=>n+p.distanceTo(points[j]),0);world.add(g);pedestrians.push(g)}
