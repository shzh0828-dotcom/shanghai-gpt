...
export function addNightSkyline(world:T.Group,groups:Map<string,T.Group>){
 const palette=['#8ab9da','#a4bfd0','#d3b68c','#83bdc5','#d7bc92','#aac2d0'];
 ...
 return {update(n:number,landmarkBoost=1){accents.forEach(m=>m.emissiveIntensity=n*2.1);surfaceLights.forEach(({m,strength})=>m.emissiveIntensity=n*strength*landmarkBoost);screenMaterial.emissiveIntensity=n*1.55*landmarkBoost;screen.visible=n>.05;}};
}
