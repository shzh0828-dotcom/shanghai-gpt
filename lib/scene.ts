...
export function createScene(host:HTMLElement,onSelect:(id:string)=>void,onManual:()=>void):SceneAPI{
 ...
 const cars:T.Group[]=[];for(let i=0;i<180;i++){const car=new T.Group();car.name='Vehicle '+(i+1);box(car,0,.3,0,.42,.3,1.05,i%4===0?gold:trim);box(car,0,.53,-.08,.37,.2,.55,glass);for(const side of [-1,1]){box(car,side*.15,.33,.54,.09,.09,.03,lampMat);for(const z of [-.32,.32])box(car,side*.23,.18,z,.07,.16,.16,dark);}world.add(car);cars.push(car)}
